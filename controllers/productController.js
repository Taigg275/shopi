...
export const createProductController = async (req, res) => {
    try {
        ...
        switch (true) {
            ...
            case !category:
                return res.status(500).send({ error: 'Vui lòng nhập danh mục sản phẩm' });
            case !quantity:
                return res.status(500).send({ error: 'Vui lòng nhập số lượng sản phẩm' });
            case photo && photo.size > 1000000:
                return res.status(500).send({ error: 'Ảnh là bắt buộc và phải nhỏ hơn 1mb' });
        }
...
export const getProductController = async (req, res) => {
    try {
        ...
        res.status(500).send({
            success: false,
            message: 'Lỗi khi xem tất cả sản phẩm',
            error: error.message,
        });
    }
};
...
export const getSingleProductController = async (req, res) => {
    try {
        ...
        res.status(200).send({
            success: true,
            message: 'Đã tìm nạp một sản phẩm',
            product,
        });
    } catch (error) {
        console.log(error);
...
export const productListController = async (req, res) => {
    try {
        ...
        res.status(400).send({
            success: false,
            message: 'Lỗi trên mỗi trang ctrl',
            error,
        });
    }
};
